    ...
    export default {
        ...
        methods: {
            ...
            doInvoke(index) {
                ...
                let currentPage = pages[curIndex];
                let offset = Math.abs(index - curIndex);
                if(index === curIndex) {
                  return false;
                }
                if(index > curIndex) {
                ...
                this.doAnimate(direction,{},offset);
            },

            doAnimate(towards, options={},step=1) {
                ...
                if (!Object.keys(options).length) {
                    ...
                    prevPage = pages[index - step];
                    nextPage = pages[index + step];
                    if (this.continuous && pages.length > 1) {
                        if (!prevPage) {
                            prevPage = pages[pages.length - step];
                        }
                        if (!nextPage) {
                            nextPage = pages[0];
                        }
                ...
                if (towards === 'prev') {
                    ...
                    if (this.continuous && index === 0) {
                        newIndex = pageCount - step;
                    }
                } else if (towards === 'next') {
                    if (index < pageCount - step) {
                        newIndex = index + step;
                    }
                    if (this.continuous && index === pageCount - step) {
                        newIndex = 0;
                    }
                }
                ...
                var callback = () => {
                    ...
                    this.$emit('swipe-invoke',{index: this.index});
                    if (prevPage) {
